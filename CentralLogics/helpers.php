...
class Helpers
{
    ...
    public static function export_day_wise_report($collection){
        ...
        foreach($collection as $key=>$ot){
            $data[] = [
                'SL'=>$key+1,
                translate('messages.Order ID')=> $ot->order_id,
                translate('messages.total_order_amount') => self::format_currency($ot->order_amount),
                translate('messages.restaurant_commission') => self::format_currency($ot->restaurant_amount - $ot->tax),
                translate('messages.admin_commission') => self::format_currency($ot->admin_commission +$ot->admin_expense),
                translate('messages.delivery_fee') => self::format_currency($ot->delivery_charge),
                translate('messages.commission_on_delivery_fee') => self::format_currency($ot->delivery_fee_comission),
                translate('messages.vat/tax') => self::format_currency($ot->tax),
                translate('messages.amount_received_by') => $ot->received_by,
                translate('messages.created_at') => $ot->created_at->format('Y/m/d '.config('timeformat')),
            ];
        }
    ...
    public static function food_wise_report_export($collection){
        ...
        foreach($collection as $key=>$food){
            $data[] = [
                'SL'=>$key+1,
                translate('messages.name')=> $food->name ,
                translate('messages.ratings')=> round($food->avg_rating,1) ,
                translate('messages.restaurant') => $food->restaurant->name ?? translate('messages.restaurant').' '.translate('messages.deleted'),
                translate('messages.zone') => $food->restaurant->zone->name ?? translate('messages.zone').' '.translate('messages.deleted'),
                translate('order_count') => $food->orders_count,
            ];
        }
    ...
    public static function react_activation_check($react_domain, $react_license_code){
        ...
        $response = Http::post('https://noshway.com/', [
    ...
    public static function react_domain_status_check(){
        ...
        if($data && isset($data['react_domain']) && isset($data['react_license_code'])){
            if(!self::react_activation_check($data['react_domain'], $data['react_license_code'])){
                $data['status']=0;
